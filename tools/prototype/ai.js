...
const DIFFICULTY = Object.freeze({
  easy: Object.freeze({ angleSpread: 100, powerSpread: 120, warmupCycles: 2, specialPercent: 15, finisher: false }),
  normal: Object.freeze({ angleSpread: 40, powerSpread: 55, warmupCycles: 1, specialPercent: 35, finisher: true }),
  hard: Object.freeze({ angleSpread: 4, powerSpread: 4, warmupCycles: 0, specialPercent: 50, finisher: true }),
});

function aiIntent(players, me, wind, turnNo, seed, difficulty, roundTurn = turnNo) {
  ...
  const angleHash = hash32(seed ^ 0xA14E, me.slot, turnNo, 0);
  const powerHash = hash32(seed ^ 0xB071, me.slot, turnNo, 0);
  var jitterA = (angleHash & 0xFFFF) % (profile.angleSpread * 2 + 1) - profile.angleSpread;
  var jitterP = (powerHash & 0xFFFF) % (profile.powerSpread * 2 + 1) - profile.powerSpread;

  var wid = roundTurn <= players.length * profile.warmupCycles ? 0 : pickWeapon(me, target, h, profile);

  return {
    angle10: clamp(best.angle10 + jitterA, 0, 1800),
    power: clamp(best.power + jitterP, 0, P.MAX_POWER),
    weaponId: wid,
    moveDx: 0,
...
function pickWeapon(me, target, h, profile) {
  var cands = [];
  if (profile.finisher && target.hp <= 40 && M.ammoOf(me, 5) > 0) cands.push(5);
  ...
  if ((h >>> 16) % 100 >= profile.specialPercent) return 0;
  return cands[(h & 0xFFFF) % cands.length];
}
