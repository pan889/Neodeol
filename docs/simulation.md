...
`WIND_SCALE_Q8 = 192`이므로 수평 가속의 평균은 표시 바람의 75%다.
`magnitude = abs(wind) * WIND_SCALE_Q8`로 두고 틱 `t`(0부터)의 가속은
`sign(wind) * (((magnitude * (t + 1)) >> 8) - ((magnitude * t) >> 8))`이다.
약한 바람 ±1도 4틱에 ±3만큼 누적되어 소실되지 않으며 음수만 더 강하게 내림하지 않는다.
각 탄도 구간의 시작에서 `t = 0`으로 시작한다. 분열 후 자탄은 부모의 정수 속도를 이어받되
...
| `POWER_MAX` | **1500** | 파워 | B22 — 최대 역풍 45° 사거리 3230px, 무풍 4307px. `v0 = 936000 >> 10 = 914` |
| `WIND_MAX` | **4** | 표시 바람 단위 | 75% 계수 적용 전 최대치. §8.2 |
| `WIND_SCALE_Q8` | **192** | Q8 | 수평 가속 75%. 최대 실효 가속 3 subpx/tick² |
| `DRAG` | 0 | Q16 | §4.5 |
...
`WIND_MAX * (WIND_SCALE_Q8 / 256) / GRAVITY`에 가깝고 파워와 거의 무관하다.
...
표시 범위는 **4**를 유지하되 2026-09-17 피드백으로 `WIND_SCALE_Q8 = 192`를 도입했다.
...
음수는 발사 위치의 왼쪽에 떨어지는 경우다. 높은 각도에서는 여전히 바람 보정이 필요하지만,
체공 시간·중력·무풍 탄도는 바꾸지 않고 바람으로 생기는 편차만 약 25% 줄였다.
...
- [x] `WIND_MAX = 4`, `WIND_SCALE_Q8 = 192` — 탄도 영향 75%, 60% 유지·최대 ±1 변화. §8.2, `decisions.md` C4
