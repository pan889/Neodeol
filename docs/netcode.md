...
- [x] **상수는 같고 `sim/` 코드만 다른 빌드를 무엇이 잡는가** (§7.3) — 지문·`SIM_VERSION` 은
      **값**만 해시하므로 절차만 바뀐 빌드는 못 잡는다. 그건 원래 지문의 일이 아니고
      **골든 리플레이가 잡는다** (`CLAUDE.md` §결정론 게이트). 상수가 그대로인 `sim/` 변경은
      `RULES_VERSION` 을 손으로 올리고, 그걸 잊었을 때의 두 번째 그물이 골든이다 —
      지금 25개(자동자 20 · 장기 2 · 발사 · 맵생성 · 매치)가 스텝·턴 두 단위로 대조한다
- [ ] **한 턴 재생 길이의 상한과 §3.1 의 8초** — 비행 상한 30초(`MAX_FLIGHT_TICKS = 1800`)에
      단발의 다중 자탄 정착 시간까지 더하면 8초 상한으로 담기지 않는 경우가 있다
- [x] **"직전 턴 값"이 없을 때의 폴백** (§5.3, §6) — `normalizeIntent` 가 답한다.
      각도·파워는 **플레이어에 남아 있는 현재 값**으로 떨어지고 (`makePlayer` 초기값은
      슬롯 짝수 45.0° · 홀수 135.0° · 파워 600), 무기는 보유 탄약이 없으면 표준탄(0)으로,
      이동은 0, 차폐막은 미사용으로 떨어진다. 라운드 첫 턴·부활 직후에도 값이 항상 존재한다
