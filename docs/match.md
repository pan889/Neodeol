...
라운드는 전멸·최후 1인, 또는 **턴 상한**으로 끝난다. 상한은 `ROUND_TURN_CAP`(40)을
**플레이어 수로 나누어떨어지게 내림한 값**이다.

| 인원 | 2 | 3 | 4 | 5 | 6 |
|---|---:|---:|---:|---:|---:|
| 유효 상한 | 40 | 39 | 40 | 40 | 36 |

> **왜 내림하는가.** 상한은 개별 발사 횟수이고, 캡으로 끝난 라운드의 승자는 최고 HP 로
> 정해진다. 40 이 3으로 안 나뉘면 앞쪽 슬롯이 한 발을 더 쏘고, 그게 그대로 슬롯 유불리가 된다.
>
> 라운드 선공(`(roundNo - 1) % playerCount`)의 편차는 5라운드가 6인으로 안 나뉘어 남는다.
> 그건 라운드 수를 인원마다 바꿔야 해서 매치 길이가 들쭉날쭉해지므로 감수한다. `decisions.md` B7.

> ⚠ **이 분기는 한동안 죽은 코드였고, 그 안에 실제 이탈이 숨어 있었다.**
> Python 이 `round_turn_cap * 1000`(=40,000), TS 가 `roundTurnCap`(=40)을 써서
> **턴 40 에서 TS 는 라운드를 끝내고 Python 은 계속 갔다.** match 골든이 턴 11 까지만
> 가서 교차 검증이 못 잡았다. 밟지 않는 분기는 대조되지 않는다.

### 5.2 피해 귀속

- 직접 피해는 각 폭발 소유자에게 귀속한다.
- **정산은 실제로 깎인 HP 만큼이다.** 계산상 피해 전량을 주면 빈사 상태 적에게 큰 무기를
  맞혔을 때 기여의 수십 배를 받는다 — 남은 HP 3 에 핵포탄(120)이면 960G·120점이었다.
- 차폐막은 첫 피격 한 번을 막고 즉시 내려간다. **피해가 0 인 히트는 소모하지 않는다** —
  반경 경계는 정수 시프트 때문에 `dmg = 0` 이 나오고(반경 1024 무기의 1020~1023),
  그걸로 600G 아이템이 벗겨지면 일부러 끄트머리에 떨어뜨리는 것이 공짜 해제가 된다.
- 낙하 피해는 이번 턴 **마지막 폭발의 소유자**에게 귀속한다.
  스스로 걸어 내려간 낙하는 귀속 대상이 없다 (§5.1).
- **매몰 피해는 묻은 사람에게 귀속한다.** 매몰은 과거 턴에 생긴 지속 상태라 귀속도 그때
  정해진다 (`buriedBy`). 매 턴 발사자로 갈아끼우면 남이 묻어놓은 적 쪽으로 아무 데나
  쏘기만 해도 턴당 피해와 처치 크레딧을 가져간다.
- 사망 귀속은 **마지막 타격을 넣은 쪽**이다. 낙하는 그 턴의 발사자, 매몰은 묻은 사람이며
  둘이 다를 수 있다.
