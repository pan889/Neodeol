...
조준 중에는 **탱크 주변에 각도 다이얼**이 뜬다. 이것만이 화면 중앙부에 나타나는 UI다.
확정하면 즉시 사라진다.
...
- [ ] 탱크 실루엣 디자인 — 팀색 액센트만 쓰고 본체는 어둡게 (미정)
...
- [ ] **입자 발생원** — §5 는 "이동한 셀" 목록을 전제하는데 `terrain.md` §5.2 가
      "상태는 격자 하나뿐"으로 정해 그 목록을 내주는 곳이 없다. `terrain.md` §10 미결 11
